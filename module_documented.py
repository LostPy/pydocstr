...
def additions(a: int, b: int) -> int:
	"""Une fonction pour additioner deux nombres.
...
def somme(list_: List[int]) -> int:
	"""Une fonction pour réaliser la somme d'une liste.
...
def get_positions() -> Tuple[int, int]:
	"""Une fonction qui donne la positions d'un point
...
def default_args_example(a, b=3, c: str = "Hello World"):
	"""Une fonction avec des arguments non typé et avec une valeurs par défault
...
def function_with_multi_lines(a, b, c, d, e, f, g: int, h: str,
								...
								l: int = 3, m: int = 2):
	"""Fonction définie sur plusieurs lignes
